...
class App(CTk.CTk):
    ...
    def start_key_generation(self):
        ...
        if self.is_generating:
            print("[Info] Key generation already running!")
            return
...
def update_entropy_graph(entropy_value):
    """Update entropy graph - now thread-safe"""
    entropy_history.append(entropy_value)
    
    # Only update graph every 10 frames to reduce overhead
    if len(entropy_history) % 10 == 0:
        try:
            plt.clf()
            ...
            pass  # Ignore matplotlib errors in thread
...
def test_key_randomness(file_name, max_keys=None):
    ...
    for index in range(num_keys):
        try:
            ...
            # Extract hex key (handle both formats)
            if ':' in key_hex_full:
                key_hex = key_hex_full.split(':')[1].strip().replace('"', '')
            else:
                key_hex = key_hex_full.strip().replace('"', '')
            ...
            if len(key_hex) < 32:  # Less than 128 bits
                skipped_keys += 1
                if skipped_keys <= 5:  # Show first 5 errors
                    print(f"  [Warning] Key {index} too short ({len(key_hex)*4} bits) - skipping")
                continue
            
            # Convert to binary with proper padding (each hex char = 4 bits)
            try:
                binary = bin(int(key_hex, 16))[2:].zfill(len(key_hex) * 4)
            except ValueError as e:
                skipped_keys += 1
                if skipped_keys <= 5:
                    print(f"  [Warning] Key {index} invalid hex: {e} - skipping")
                continue
            
            # Verify we have enough bits for NIST tests (minimum 128 bits)
            if len(binary) < 128:
                skipped_keys += 1
                if skipped_keys <= 5:
                    print(f"  [Warning] Key {index} binary too short ({len(binary)} bits) - skipping")
                continue
            ...
            if (valid_keys) % 100 == 0:
                print(f"  Processed {valid_keys} valid keys ({index + 1}/{num_keys} total)...")
                
        except Exception as e:
            skipped_keys += 1
            if skipped_keys <= 5:
                print(f"  [Error] Key {index}: {e}")
            continue

    # Check if we have any valid results
    if valid_keys == 0:
        print("\n" + "="*60)
        print("NO VALID KEYS TO TEST")
        print("="*60)
        print(f"Total keys in file: {num_keys}")
        print(f"Keys skipped: {skipped_keys}")
        print("\n[Possible Issues]:")
        print("  • Keys may be corrupted or in wrong format")
        print("  • Entropy values are too low (check key generation)")
        print("  • CSV file may need to be regenerated")
        print("="*60 + "\n")
        return None
    ...
    print("="*60)
    print(f"Valid keys tested: {valid_keys}/{num_keys}")
    if skipped_keys > 0:
        print(f"Keys skipped: {skipped_keys} (invalid/too short)")
    print()
    ...
    print("\n" + "="*60)
    print(f"[Testing complete! Tested {valid_keys} valid keys]")
    print("="*60 + "\n")
    
    return all_results  # Return results for further analysis

# --------------------------------
# Main loop with ESC key fix
# --------------------------------           
def main():
    global stop_generation
    ...
    # Initialize matplotlib for entropy graph
    try:
        plt.ion()
        plt.figure(figsize=(8, 4))
    except:
        print("[Warning] Could not initialize matplotlib graph")
    ...
    print("\n[✓] Key generation started. OpenCV window will appear...")
    ...
    while not stop_generation:
        if not paused:
            ...
            # Update graph (thread-safe version)
        ...
        if frame_counter != 0:
            # Still need to check for key press even when skipping frames
        ...
        # Key controls (waitKey is CRITICAL for OpenCV window events)
        keypress = cv.waitKey(100) & 0xFF
    ...
    # End of main loop: release webcam and cleanup
    print("\n[Cleaning up...]")
    cap.release()
