...
def test_key_randomness(file_name):
    """
    Reads all generated keys from the CSV, performs the Monobit-style 
    randomness test on each key, and reports the average score.
    """
    ...
    # Initialize list to hold scores for all keys
    all_scores = []
    
    # 2. Iterate through all keys (instead of selecting one random key)
    # df.iterrows() loops through every row in the DataFrame
    for index, row in df.iterrows():
        try:
            ...
            balance = abs(zeros - ones) / 256
            randomness_score = 1 - balance
            
            all_scores.append(randomness_score)
    ...
    print("-" * 40)
    print(f"   Randomness Test Results for {len(all_scores)} Keys:")
    print(f"   Average Monobit Score: **{average_score:.4f}**")
    print(f"   Lowest Score Found:    **{min(all_scores):.4f}**")
    print("-" * 40)
