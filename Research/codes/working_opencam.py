...
class App(CTk.CTk):
    ...
    def button_test(self):
        button = CTk.CTkButton(self.main_frame, 
                            ...
                            border_color="#AFD48D",
                            font=CTk.CTkFont(size=20),
                            command=lambda:threading.Thread(target=lambda: test_key_randomness("enc.csv"), daemon=True).start())
        button.pack(pady=30)
    ...
    def button_graphs(self):
        button = CTk.CTkButton(self.main_frame, 
                            ...
                            border_color="#AFD48D",
                            font=CTk.CTkFont(size=20), 
                        command=lambda: threading.Thread(target=lambda: generate_advanced_graphs("enc.csv"), daemon=True).start())
        button.pack(pady=30)
...
def test_key_randomness(file_name, max_keys=None):
    ...
    print("\n" + "="*60)
    print("AGGREGATE RESULTS ACROSS ALL KEYS FOR OPENCAM")
    print("="*60)
